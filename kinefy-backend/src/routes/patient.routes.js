...
router.post('/', authMiddleware, checkRole('fisioterapeuta'), patientController.createPatient);

// @route   GET /api/patients
// @desc    Obtener todos los pacientes del fisioterapeuta
// @access  Privado
router.get('/', authMiddleware, checkRole('fisioterapeuta'), patientController.getPatients);
...
// @access  Privado
router.put('/:id', authMiddleware, checkRole('fisioterapeuta'), patientController.updatePatient);

// @route   DELETE /api/patients/:id
// @desc    Eliminar un paciente
// @access  Privado
router.delete('/:id', authMiddleware, checkRole('fisioterapeuta'), patientController.deletePatient);

// @route   POST /api/patients/:id/reset-password
// @desc    Generar contraseña temporal para el paciente
// @access  Privado (Solo Fisioterapeutas)
router.post('/:id/reset-password', authMiddleware, checkRole('fisioterapeuta'), patientController.resetPatientPassword);

// @route   POST /api/patients/:id/exercises
// @desc    Asignar ejercicios a un paciente
// @access  Privado (Fisios)
router.post('/:id/exercises', authMiddleware, checkRole('fisioterapeuta'), patientController.assignExercises);
...
// @access  Privado (Fisios)
router.post('/:id/documents', authMiddleware, checkRole('fisioterapeuta'), patientController.addDocument);

// @route   DELETE /api/patients/:id/documents/:documentId
// @desc    Eliminar un documento de la ficha
// @access  Privado (Fisios)
router.delete('/:id/documents/:documentId', authMiddleware, checkRole('fisioterapeuta'), patientController.deleteDocument);
